                ...
                <form action="{{ route('sales.store') }}" method="POST" id="sales-form">
                    ...
                    <div class="row">
                        <div class="col-md-3 mb-3">
                            ...
                            <input type="date" id="sale_date" name="sale_date" class="form-control" value="{{ old('sale_date', date('Y-m-d')) }}" required>
                            @error('sale_date')
                        ...
                        <div class="col-md-2 mb-3">
                            ...
                            <select id="sale_mode" name="sale_mode" class="form-control" required>
                                @foreach (['Cash', 'Credit', 'UPI'] as $mode)
                                    <option value="{{ $mode }}" {{ old('sale_mode', 'Cash') === $mode ? 'selected' : '' }}>{{ $mode }}</option>
                                @endforeach
                            </select>
                            @error('sale_mode')
                                <small class="text-danger">{{ $message }}</small>
                    ...
                    <tbody>
                        @forelse ($sales as $sale)
                            <tr>
                                ...
                                <td class="text-right">{{ number_format((float) ($sale->discount_amount ?? 0), 2) }}</td>
                                <td class="text-right">{{ number_format($sale->total_amount, 2) }}</td>
                                <td class="text-nowrap">
                                    @if ($canEdit)
                                        <a href="{{ route('sales.edit', $sale) }}" class="btn btn-sm btn-info">Edit</a>
                                    @endif
                                    @if ($canDelete)
...
(function () {
    ...
    function addRow() {
        ...
        row.innerHTML = ''
            ...
            +   '<input type="text" class="form-control product-name-input" list="product-name-list" placeholder="Type or select product" required>'
            + '</td>'
