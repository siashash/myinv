        ...
        <div class="card-body">
            <form method="GET" action="{{ route('reports.stock') }}" class="row">
                ...
                <div class="col-md-4 mb-3 d-flex align-items-end">
                    ...
                    <a href="{{ route('reports.stock') }}" class="btn btn-secondary btn-sm">Reset</a>
                </div>
            </form>
        </div>
    </div>
                    ...
                    <tbody>
                        @forelse ($rows as $row)
                            <tr>
                                ...
                                <td class="text-right">{{ number_format($row->movement_qty, 3) }}</td>
                                <td class="text-right">{{ number_format($row->opening_stock, 3) }}</td>
                                <td class="text-right">{{ number_format($row->purchase_qty, 3) }}</td>
                                <td class="text-right">{{ number_format($row->sales_qty, 3) }}</td>
                                <td class="text-right">{{ number_format($row->purchase_return_qty, 3) }}</td>
                                <td class="text-right">{{ number_format($row->sales_return_qty, 3) }}</td>
                                <td class="text-right font-weight-bold">{{ number_format($row->current_stock, 3) }}</td>
                            </tr>
                        @empty
                            <tr>
                    ...
                    <table class="table table-bordered mb-0">
                        <thead class="table-light">
                            <tr>
                                ...
                                <th class="text-right">Purchase Return Qty</th>
                        ...
                        <tbody>
                            @forelse ($productMovements as $movement)
                                <tr>
                                    ...
                                    <td class="text-right">{{ number_format($movement['purchase_qty'], 3) }}</td>
                                    <td class="text-right">{{ number_format($movement['purchase_return_qty'], 3) }}</td>
                                    <td class="text-right">{{ number_format($movement['net_qty'], 3) }}</td>
                                    <td class="text-right font-weight-bold">{{ number_format($movement['balance_stock'], 3) }}</td>
                                </tr>
                            @empty
                                <tr>
